        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <div class="row">
                    ...
                    <div class="col-8"><input type="search" class="form-control form-control-sm" wire:model.debounce.300ms="searchTerm" placeholder="Search"/></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <table class="table table-hover">
			...
			<tbody>
				@foreach($data as $item)
					<tr>
						<td>{{ $item->name }}</td>
                        <td>{{ isset($item->parent) ? $item->parent->name: __('product_categories.parent') }}</td>
                        <td>{{ ($item->is_active==1) ? __('global.yes') : __('global.no') }}</td>
						<td class="col-1"><select class="form-select form-select-sm" wire:change="doAction({{ $item->id }}, $event.target.value)" id="action_{{ $item->id }}" onchange="setTimeout(()=>{ $('#action_{{ $item->id }}').val('');},500)">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="col-md-9">
                            <select class="form-select form-select-sm select @error('parent_id') is-invalid @enderror"
                                id="parent_id" wire:model.defer="state.parent_id">
